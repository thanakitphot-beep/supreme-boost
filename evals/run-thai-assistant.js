...
async function main() {
    ...
    const dataset = curriculum ? require('./agent-training.cases').buildTrainingCorpus() : cases;
    const selected = dataset.filter(test => (split === 'all' || test.split === split) && (!caseId || test.id === caseId));
    ...
    try {
        for (const test of selected) {
            ...
            const result = pipeline ? await runPipeline({
                ...
                siteDNA: { entityIndex: curriculum ? [] : [
    ...
    const report = {
        ...
        datasetHash: crypto.createHash('sha256').update(JSON.stringify(dataset)).digest('hex'),
        curriculum,
