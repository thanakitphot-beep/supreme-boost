...
const { sanitizeCapabilities, safeClientAction } = require('../services/ai/clientCapabilities');
...
async function askBrain(payload, mergedHistory, requestId) {
    ...
    return indicatorAI.generate({
        ...
        toolContext: { tenantId: payload.tenantId, payload },
        metadata: { requestId },
...
async function runOwnedPipeline(payload, mergedHistory, requestId) {
    const exactAnswer = !payload.isProactive && verifiedAnswer(payload);
    ...
    const reasoningUnavailable = () => ({
        reply: 'การเชื่อมต่อผู้ช่วย AI ขัดข้องชั่วคราวครับ ลองส่งอีกครั้ง หรือระบุชื่อสินค้า/หน้าที่ต้องการเพื่อให้ผมช่วยค้นจากข้อมูลเว็บไซต์ได้',
...
async function handler(req, res) {
    ...
    try {
        ...
        const payload = {
            ...
            tenantId,
            clientCapabilities: sanitizeCapabilities(body.clientCapabilities),
            tenantSettings,
            siteProfile
        };
        ...
        result.action = safeClientAction(result.action, payload.clientCapabilities);
